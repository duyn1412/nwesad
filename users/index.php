...
include __DIR__ . '/user-admin.php';
    ...
    ?>
      ...
      <div class="d-flex align-items-start">
          <div class="nav flex-column nav-pills col-sm-3" id="v-pills-tab" role="tablist" aria-orientation="vertical">
            <button class="nav-link <?php echo $action != 'add_user' ? 'active' : '' ?> gap-3" id="v-pills-home-tab" data-bs-toggle="pill" data-bs-target="#v-pills-home" type="button" role="tab" aria-controls="v-pills-home" aria-selected="true">Existing Users</button>
            <button class="nav-link gap-3 <?php echo $action == 'add_user' ? 'active' : '' ?>" id="v-pills-profile-tab" data-bs-toggle="pill" data-bs-target="#v-pills-profile" type="button" role="tab" aria-controls="v-pills-profile" aria-selected="false">Add User</button>
           

          </div>
         
          <div class="tab-content col-sm-9" id="v-pills-tabContent">
            <div class="tab-pane fade <?php echo $action != 'add_user' ? ' show active' : '' ?>" id="v-pills-home" role="tabpanel" aria-labelledby="v-pills-home-tab">

            
            <?php
              $sql = "SELECT username FROM nwengine_user";
             // $result = $conn->query($sql);
              ...
              if ($stmt->num_rows  > 0) {
                  ...
                  while( $stmt->fetch()) {
                      ...
                      echo '</div>';
                      echo '<div class="col-md-6">';
                      echo '<button type="button" class="btn btn-primary edit-password" data-bs-toggle="modal" data-bs-target="#editPasswordModal" data-username="' . $username . '">
                      Edit Password
                    </button>';
                      // echo '<form method="post" action="" class="edit-password-form" style="display: none;">';
                      // echo '<input type="hidden" name="username" value="' . $username . '">';
                      // echo '<label for="password">New Password:</label>';
                      // echo '<input type="password" required class="form-control" id="password" name="password">';
                      // echo '<button type="submit" name="action" value="update_password" class="btn btn-primary">Update Password</button>';
                      // echo '</form>';
                      echo '<form method="post" action="" class="d-inline">';
                      echo '<input type="hidden" name="username" value="' . $username . '">';
                      if($username != 'admin'){

                        echo '<button type="submit" name="action" value="remove_user" class="btn btn-danger">Remove User</button>';
                ...
                if (isset($_SESSION['message']) && $action != 'add_user') {
            ...
            <div class="tab-pane fade <?php echo $action == 'add_user' ? ' show active' : '' ?>" id="v-pills-profile" role="tabpanel" aria-labelledby="v-pills-profile-tab">
...
$(document).ready(function() {
  $('#editPasswordModal').on('show.bs.modal', function (event) {
   ...
   // modal.find('.modal-body #d-username').val(username).change();
    //console.log(modal.find('.modal-body #d-username'));
  });
});
</script>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
          ...
          <div class="mb-3">
            ...
            <input type="password" required name="password" class="form-control" id="dpassword">
          </div>
          <!-- <div class="mb-3">
      ...
      <input type="hidden" id="d-username" name="username" value="">
        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
        <button type="submit" name="action" value="update_password" class="btn btn-primary">Save changes</button>
      </div>
      </form>
    </div>
  </div>
